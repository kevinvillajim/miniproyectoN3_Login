...
# Funcionalidades del proyecto terminado:

- Diseño identico a Figma.
- Dark y ligth mode con Local Storage, se mantiene en todas las vistas y se puede cambiar desde el switch en login e index y desde el modal del perfil (realizado con JS).
- Contraseña hasheada y comprobacion con db.
- En la comprobación, define si está incorrecto el login en caso que haya un usuario no registrado muestra que No ha sido registrado tal usuario y en caso de estarlo pero que la contraseña no coincida muestra Contraseña incorrecta.
- En el registro, no se permite ingresar un usuario ya registrado.
- No se permiten ingresar campos vacios en el index por metodo required html y por un if en el php.
- Imagenes guardadas en la base de datos.
  -En caso que no haya una imagen subida aún, tiene una imagen de perfil por defecto.
- Diseño responsive con las mismas funcionalidades de la vista de escritorio.

[DEPLOY LINK](https://miniproyecton3kevinvillajim.000webhostapp.com/)
